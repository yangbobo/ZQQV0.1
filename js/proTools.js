...
/* 内容滚动 */
function MarqueeBox(opts) {
	this.obj = opts.obj;
	this.speed = opts.speed || 30;
	this.direction = opts.direction || "left";
	this.init();
}
MarqueeBox.prototype = {
	init: function() {
		var self = this;
		if (self.direction == "left") {
			self.obj.addClass("alinetxt");
			if (this.obj.find("span").width() > this.obj.width()) {
				this.obj.html(this.obj.html() + this.obj.html());
			} else {
				return false;
			}
			...
			self.obj.addClass("alinetxt-nowrap");
			self.runTop();
		}
	...
	runLeft: function() {
		var self = this;
		var ele = self.obj;
		var eleCon = self.obj.find("span");
		clearInterval(self.timer);
		self.timer = setInterval(function() {
			ele.scrollLeft(ele.scrollLeft() + 1);
			if (ele.scrollLeft() >= eleCon.width()) {
				ele.scrollLeft(0);
			}
		}, self.speed);
	...
	runTop: function() {
		...
		clearInterval(self.timer);
		self.timer = setInterval(function() {
			mTop = parseInt(eleCon.css("margin-top"));
			mTop += -16;
			if (eleCon.height() - Math.abs(mTop) == 0) {
				eleCon.css("margin-top", 0);
			} else {
				eleCon.css("margin-top", mTop + "px");
			}
